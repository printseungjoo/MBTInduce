...
export default function EditSimulation({ target, id }: EditSimulationProps) {
    ...
    async function editUserName(userName: string) {
        try {
            const response = await fetch(`http://localhost:4000/api/simulation/userProfiles/${id}`, {
                method: 'PATCH',
                credentials: 'include',
                headers: {
    ...
    async function editSimulationContent(simulationContent: string) {
        try {
            const response = await fetch(`http://localhost:4000/api/simulation/simulationTemplate/${id}`, {
                method: 'PATCH',
                credentials: 'include',
                headers: {
                    'Content-Type': 'application/json',
